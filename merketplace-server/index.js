...
async function run() {
    try {
        ...
        const paypalEmailCollections = client.db('marketPlaceDB').collection('paypalEmail');
        const commissionCollections = client.db('marketPlaceDB').collection('commission');
        const logoCollections = client.db('marketPlaceDB').collection('logo');
        const BannerOptionCollections = client.db('marketPlaceDB').collection('Banner');
        const supportMessageCollections = client.db('marketPlaceDB').collection('supportMessage');
        const ContactMessageCollections = client.db("marketPlaceDB").collection("contactMessage");

        /* Seller */
        const productsCollection = client.db('marketPlaceDB').collection('products');
        const categoriesCollection = client.db('marketPlaceDB').collection('categories');
        const withdrawCollection = client.db('marketPlaceDB').collection('withdraw');
        ...
        */
        ...
        app.post('/add-withdraw', async (req, res) => {
            try {
                ...
                const seller = await usersCollection.findOne({ UserEmail: withdrawalUserEmail });
                if (!seller) {
                ...
                const updatedBalance = parseFloat(seller.currentBalance) - parseFloat(withdrawalAmount);
                await usersCollection.updateOne({ UserEmail: withdrawalUserEmail }, { $set: { currentBalance: updatedBalance } });
